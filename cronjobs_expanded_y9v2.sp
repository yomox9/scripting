...
{
	name = "Cronjobs by yomox9 for AliveMonitoring y9v2 forSrv01",
	author = "Popoklopsi. Customized yomox9",
	version = "1.2",
	description = "A cronjob Plugin for Sourcemod",
	url = "https://forums.alliedmods.net/showthread.php?t=142073"
};

public OnPluginStart()
{
	CreateConVar("crontab_version", "1.1", "Crontab by Popoklopsi", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	
	c_logging = CreateConVar("crontab_logging", "1", "Logging Crontab executions and outputs", FCVAR_PLUGIN);
	
	loadCrons();
	
	CreateTimer(0.1, doJobs, _);
	CreateTimer(30.0, doJobs, _, TIMER_REPEAT);
	
	RegServerCmd("cronjobs_reload", reloadCrons, "Reload the Crontab");
	
	AutoExecConfig(true, "cronjobs", "");
}
...
public loadCrons()
{
	if (FileExists("cfg/cronjobs_AM_y9v1_srv01.txt"))
	{
		decl String:fileLine[2700];
		decl String:splited[2024];
		
		new Handle:file = OpenFile("cfg/cronjobs_AM_y9v1_srv01.txt", "rb");
		...
		while(ReadFileLine(file, fileLine, sizeof(fileLine)))
		{
			...
			if (!StrEqual(fileLine, "\0") && strlen(fileLine) > 10 && (fileLine[0] != '/' || fileLine[1] != '/'))
			{
				...
				if (!error)
				{
					...
					LogMessage("CronJob '%s' successfully added!", fileLine);
				}
				else
				{
					for (new x=0; x < 60; x++)
					...
					LogError("CronJob '%s' couldn't added!", fileLine);
				}
			}
		}
	}
		...
		if (execute)
		{
			if (StrEqual(cron_style[i], "console"))
			{
				if (logging)
				{
					...
					LogMessage("Executed Crontab Console Command: %s", cron_command[i]);
					LogMessage("Crontab Output is: %s", output);
				}
				else
			...
			else if (StrEqual(cron_style[i], "player"))
			{
				...
				if (logging)
					LogMessage("Executed Crontab Player Command: %s", cron_command[i]);
			}
			else if (StrEqual(cron_style[i], "system"))
			{
				if (LibraryExists("system2"))
					System2_RunThreadCommand(CommandRun, cron_command[i]);
				else
...
public CommandRun(const String:output[], const size, CMDReturn:status, any:data)
{
	if (logging)
	{
		//LogMessage("Crontab Output is: %s", output);
