...
{
	name = "[NMRiH]Item Shop V3 customized y9v4 forSrv04",
...
public ConnectDatabase()
{
	...
	SQL_TQuery(hDatabase, T_FastQuery, "CREATE TABLE IF NOT EXISTS nmrih_itemshop04 (steam_id VARCHAR(64) PRIMARY KEY, name TEXT, cash INTEGER);");
}
...
public OnMapStart()
{
	for(new Client=1; Client<=MaxClients; Client++)
	{
		if(IsClientInGame(Client) && IsClientAuthorized(Client))
		{
			...
			Format(query, sizeof(query), "SELECT name, cash FROM nmrih_itemshop04 WHERE steam_id = '%s' LIMIT 1;", auth);
...
public Data_Save(Client)
{
	if(ClientCash[Client] != -1)
	{
		...
		Format(query, sizeof(query), "UPDATE nmrih_itemshop04 SET cash = %d WHERE steam_id = '%s';", ClientCash[Client], authid);
...
public T_LoadPlayer(Handle:owner, Handle:hndl, const String:error[], any:Client)
{
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		else
		{
			...
			Format(query, sizeof(query), "INSERT INTO nmrih_itemshop04 VALUES ('%s', '%s', %d);", authid, escname, cash);
			SQL_TQuery(hDatabase, T_FastQuery, query);
...
public UpdatePlayerName(const String:authid[], const String:name[])
{
	...
	Format(query, sizeof(query), "UPDATE nmrih_itemshop04 SET name = '%s' WHERE steam_id = '%s';", escname, authid);
...
public CashRank(Client)
{
	...
	SQL_TQuery(hDatabase, T_UpdateTotalQuery, "SELECT COUNT(*) FROM nmrih_itemshop04;");
	
	decl String:query[1024];
	Format(query, sizeof(query), "SELECT cash FROM nmrih_itemshop04 WHERE cash > %d ORDER BY cash ASC;", ClientCash[Client]);
...
{
	SQL_TQuery(hDatabase, T_Top10Query, "SELECT name, cash FROM nmrih_itemshop04 ORDER BY cash DESC LIMIT 10;", Client);
}
