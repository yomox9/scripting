...
{
	name = "Say Sounds (including Hybrid Edition)_yomox9forTest",
...
public OnPluginStart()
{
	...
	AutoExecConfig(true, "sm_saysounds-test");
...
public Action:Load_Sounds(Handle:timer)
{
	...
	BuildPath(Path_SM,soundlistfile,sizeof(soundlistfile),"configs/saysounds-test.cfg");
	if(!FileExists(soundlistfile))
	{
		SetFailState("saysounds-test.cfg not parsed...file doesnt exist!");
	}
	else
	{
		if (listfile != INVALID_HANDLE)
