...
public Plugin:myinfo = {
	...
	description = "SDC_Tchinese_Obj. Objecvtive translate",
	version = "1.2",
...
public OnPluginStart(){
	...
	RegConsoleCmd("obj", Command_objective2, "display current objective");
...
public Action:Command_objective2(id, Args){
	...
	//yomox9 if(!strlen(ObjectiveNowText) || StrEqual(ObjectiveNowText,"NoData",false)) Format(ObjectiveNowText,sizeof(ObjectiveNowText),"There is no objective/目的はありません/無任務提示")
	//yomox9 PrintToChat(id,"C:%d",Args)
	PrintCenterText(id, ObjectiveNowText);
	//yomox9 PrintHintText(id, "objective(目的)：%s",ObjectiveNowText)
	if(!Args) PrintToChat(id,"\x04(All) objective(目的)\x01：%s",ObjectiveNowText)
	//yomox9 SendDialogToOne(id, 0, "objective(目的)：%s",ObjectiveNowText)
}
...
stock ShowCustomObjectiveText(const String:MSG[] = "",String:CallBack[] = "",size = 0,IsCommon = 0){
	...
	if (FileToKeyValues(kv, ObjCfgPath)){
		...
		if(StrEqual(ObjectiveMessage,"NoData")){
			...
			PrintToChatAll("\x04(All) objective(目的)\x01：%s",ObjectiveMessage)
...
PrintHintTextAll(String:message[]){
	...
	Format(message2,strlen(message)+10,"objective(目的)：%s",message)
	for (new i = 1; i <= 8; i++){
		if (!IsClientInGame(i) || IsFakeClient(i)) continue;
...
SendDialogToAll(String:ObjectiveMessage[]){
	...
	Format(ObjectiveMessage2,strlen(ObjectiveMessage)+10,"objective(目的)：%s",ObjectiveMessage)
	for (new i = 1; i <= 8; i++){
		if (!IsClientInGame(i) || IsFakeClient(i)) continue;
