...
{
	name = "Weather effect Customized by yomox9. forTestVersion",
...
public OnConfigsExecuted()
{
	...
	Format(configpath, sizeof(configpath), "cfg/sourcemod/weather_effect/test/%s.cfg", g_szMapPrefix);
	if (FileExists(configpath))
		ExecCfg(configpath);
	
	Format(configpath, sizeof(configpath), "cfg/sourcemod/weather_effect/test/%s.cfg", g_szMapname);
	if (FileExists(configpath))
		ExecCfg(configpath);
