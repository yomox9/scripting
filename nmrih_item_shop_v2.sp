...
#define PLUGIN_VERSION "2.0.0.7a"
...
{
	name = "[NMRiH] Item Shop (V2)",
	author = "ys24ys & Mr.Halt",
	description = "Add Item Shop (Supports MySQL and SQLite both) on NMRiH",
...
static String:Weapon_Melee[MELEE_MAX][4][64] = {
	{"me_kitknife", "Kit Knife(부엌칼)", "10", "50"},
	{"me_wrench", "Wrench(렌치)", "10", "80"},
	{"me_pipe_lead", "Lead Pipe(쇠파이프)", "10", "200"},
	{"me_crowbar", "Crowbar(쇠지레)", "10", "200"},
	{"me_bat_metal", "Baseball Bat(야구방망이)", "10", "200"},
	{"me_shovel", "Shovel(삽)", "10", "300"},
	{"me_etool", "E-Tool(야전삽)", "10", "200"},
	{"me_hatchet", "Hatchet(손도끼)", "20", "100"},
	{"me_machete", "Machete(정글도)", "20", "100"},
	{"me_sledge", "Sledgehammer(오함마)", "30", "400"},
	{"me_axe_fire", "Fire Axe(소방 도끼)", "30", "400"},
	{"me_fubar", "FUBAR(파괴망치)", "30", "450"},
	{"me_chainsaw", "Chainsaw(전기톱)", "200", "600"},
	{"me_abrasivesaw", "Abrasive Saw(연마용톱)", "180", "550"}
};

static String:Weapon_Pistol[PISTOL_MAX][4][64] = {
	{"fa_m92fs", "Berreta M92FS", "30", "100"},
	{"fa_glock17", "Glock", "30", "100"},
	{"fa_mkiii", "Ruger MKiii", "30", "100"},
	{"fa_1911", "Colt 1911", "30", "100"},
	{"fa_sw686", "S&W 686", "30", "150"}
};

static String:Weapon_Rifle[RIFLE_MAX][4][64] = {
	{"fa_mp5a3", "H&K MP5A3", "30", "300"},
	{"fa_mac10", "Mac-10", "30", "300"},
	{"fa_1022", "Ruger 10/22", "30", "250"},
	{"fa_1022", "Ruger 10/22 w/ BX25 Magazine", "30", "250"},
	{"fa_sks", "Simonov SKS", "30", "400"},
	{"fa_winchester1892", "Winchester 1892", "30", "300"},
	{"fa_m16a4", "FN M16A4", "50", "400"},
	{"fa_m16a4", "FN M16A4 w/ Carry Handle", "50", "400"},
	{"fa_cz858", "CZ858", "50", "400"},
	{"fa_fnfal", "FN FAL", "50", "450"}
};

static String:Weapon_Sniper[SNIPER_MAX][4][64] = {
	{"bow_deerhunter", "PSE Deer Hunter", "50", "150"},
	{"fa_sako85", "Sako 85", "40", "450"},
	{"fa_jae700", "JAE 700", "50", "450"}
};

static String:Weapon_Shotgun[SHOTGUN_MAX][4][64] = {
	{"fa_sv10", "Beretta Perennia SV10", "30", "350"},
	{"fa_500a", "Mossberg 500a", "30", "350"},
	{"fa_superx3", "Winchester Super X3", "30", "350"},
	{"fa_870", "Remington 870", "30", "350"}
};


static String:Weapon_Explosive[EXPLOSIVE_MAX][4][64] = {
	{"exp_grenade", "Grenade(수류탄)", "30", "100"},
	{"exp_tnt", "TNT", "80", "100"},
	{"exp_molotov", "Molotov(화염병)", "100", "100"}
};

static String:Weapon_Tools[TOOLS_MAX][4][64] = {
	{"item_bandages", "Bandages(붕대)", "30", "35"},
	{"item_first_aid", "First Aid Kit(응급치료킷)", "80", "85"},
	{"item_pills", "Pills(감염지연제)", "30", "35"},
	{"item_gene_therapy", "Gene Therapy(백신)", "100", "35"},
	{"tool_flare_gun", "Flare Gun(신호탄총)", "500", "50"},
	{"tool_barricade", "Barricade Hammer(장도리)", "5", "50"},
	{"item_maglite", "Maglite(손전등)", "5", "100"},
	{"tool_welder", "Welder(용접기)", "100", "70"},
	{"tool_extinguisher", "Fire Extinguisher(소화기)", "5", "400"},
...
public OnPluginStart()
{
	...
	sm_nmrih_itemshop_sell_ratio = CreateConVar("sm_nmrih_itemshop_sell_ratio", "0.1", "[아이템 판매]아이템 판매시 원래 가격의 몇%를 돌려받을 것인가.\n0.01 = 1%\n1.00 = 100%", FCVAR_PLUGIN, true, 0.01, true, 1.00);
	
	AutoExecConfig(true, "nmrih_itemshop");
	
	RegConsoleCmd("sm_shop", ClientCommand_Shop, "Item shop");
	RegConsoleCmd("sm_cs", ClientCommand_Cash, "캐시 정보 확인");
...
public ConnectDatabase()
{
	...
	SQL_TQuery(hDatabase, T_FastQuery, "CREATE TABLE IF NOT EXISTS nmrih_itemshop (steam_id VARCHAR(64) PRIMARY KEY, name TEXT, cash INTEGER);");
}
...
public OnMapStart()
{
	for(new Client=1; Client<=MaxClients; Client++)
	{
		if(IsClientInGame(Client) && IsClientAuthorized(Client))
		{
			...
			Format(query, sizeof(query), "SELECT name, cash FROM nmrih_itemshop WHERE steam_id = '%s' LIMIT 1;", auth);
...
{
	PrintToChatAll("\x04Welcome! \x01%N\x04's cash is \x01%d\x04, and current rank is \x01%d\x04 of \x01%d\x04.", Client, ClientCash[Client], Cash_Rank[Client], TotalPlayers);
}
...
public Data_Save(Client)
{
	if(ClientCash[Client] != -1)
	{
		...
		Format(query, sizeof(query), "UPDATE nmrih_itemshop SET cash = %d WHERE steam_id = '%s';", ClientCash[Client], authid);
...
public T_LoadPlayer(Handle:owner, Handle:hndl, const String:error[], any:Client)
{
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		else
		{
			...
			Format(query, sizeof(query), "INSERT INTO nmrih_itemshop VALUES ('%s', '%s', %d);", authid, escname, cash);
			SQL_TQuery(hDatabase, T_FastQuery, query);
...
public UpdatePlayerName(const String:authid[], const String:name[])
{
	...
	Format(query, sizeof(query), "UPDATE nmrihitemshop SET name = '%s' WHERE steam_id = '%s';", escname, authid);
...
public OnClientSayCommand_Post(Client, const String:command[], const String:sArgs[])
{
	...
	if((strcmp(text[startidx], "!상점", false) == 0) || (strcmp(text[startidx], "!샵", false) == 0) || (strcmp(text[startidx], "!store", false) == 0))
	{
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(ClientCash[Client]-DeathCash >= 0)
	{
		...
		PrintToChat(Client, "You lose \x04%d\x01 cash because you are dead.", DeathCash);
	}
	else
	{
		ClientCash[Client] = 0;
		PrintToChat(Client, "You lost \x04%d\x01 cash because you are dead.", DeathCash + (ClientCash[Client] - DeathCash));
	}
	...
	if(Client != Attacker)
	{
		...
		if(Attacker > 0 && Attacker <= 8)
		{
			if(ClientCash[Attacker]-TKCash >= 0)
			{
				...
				PrintToChat(Attacker, "You lose \x04%d\x01 cash because you killed team-mates.", TKCash);
			}
			else
			{
				ClientCash[Attacker] = 0;
				PrintToChat(Attacker, "You lose \x04%d\x01 cash because you killed team-mates.", TKCash + (ClientCash[Attacker] - TKCash));
			}
		}
	}
}
...
public CashRank(Client)
{
	...
	SQL_TQuery(hDatabase, T_UpdateTotalQuery, "SELECT COUNT(*) FROM nmrih_itemshop;");
	
	decl String:query[1024];
	Format(query, sizeof(query), "SELECT cash FROM nmrih_itemshop WHERE cash > %d ORDER BY cash ASC;", ClientCash[Client]);
...
{
	SQL_TQuery(hDatabase, T_Top10Query, "SELECT name, cash FROM nmrih_itemshop ORDER BY cash DESC LIMIT 10;", Client);
}
...
public Cash_Menu(Client)
{
	...
	Format(Line3, sizeof(Line3), "Your rank : %d/%d(%dCash for next ranking)", Cash_Rank[Client], TotalPlayers, Cash_Diff[Client]);
	
	new Handle:aPanel = CreatePanel();
	SetPanelTitle(aPanel, Title); // 타이틀
	DrawPanelText(aPanel, Line1);
	DrawPanelText(aPanel, Line2);
	DrawPanelText(aPanel, Line3);
	DrawPanelItem(aPanel, "Close");
	SendPanelToClient(aPanel, Client, Callback_Menu_Player_Class, 60);
}
...
public Shop_Menu_Buy(Client)
{
	...
	decl String:TitleFormat[256], String:MadeBy[128];
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Melee[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
				}
			}
		}
	}
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Pistol[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Rifle[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Sniper[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Shotgun[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Explosive[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
		...
		case MenuAction_Select:
		{
			...
			if(action == MenuAction_Select)
			{
				...
				if(ClientCash[param1] >= Price)
				{
					...
					PrintToChat(param1, "\x04[Item Shop]\x01Bought %s\x04 was successful.", Weapon_Tools[Num][1]);
				}
				else
				{
					PrintToChat(param1, "\x04[Item Shop]You need to have \x01%d\x04 cash more.", Price - ClientCash[param1]);
	...
	if(iWeapon > 0)
	{
		...
		if(Sell_Price > 0)
		{
			...
			if(OtherUse > 0)
			{
				...
				PrintToChat(Client, "\x04[Item Shop]Sell item was successful! \x01%d\x04Cash를 얻었습니다.", Sell_Price);
			}
		}
		else PrintToChat(Client, "\x04[Item Shop]Sell item was failed! \x01Can't sell items\x04.");
	}
	else PrintToChat(Client, "\x04[Item Shop]Sell item was failed! Pick up items for \x01sell\x04.");
}
