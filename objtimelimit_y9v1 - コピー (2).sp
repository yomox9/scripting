	...
	if(g_iTimeLeft != -1)
	{
		...
		else if(g_iTimeLeft%60 == 0 && g_iTimeLeft != 0)
		{
			if(g_iTimeLeft/60 == 1)
				...
				PrintToChat(iClient, "[SM] You have %d minutes to complete this objective before the National Guard withdraws their support and leaves without you!\nあああああ", g_iTimeLeft/60);
		}	
...
public Action:PrintTimeLeft()
{
	if(g_iTimeLeft/60 == 0 && g_iTimeLeft != 0)
	{
		...
		PrintToChatAll("今の目標を完了するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 分です", g_iTimeLeft/60);
	}
	else if(g_iTimeLeft%60 == 0 && g_iTimeLeft != 0)
	{
		if(g_iTimeLeft/60 == 1)
		{
			...
			PrintToChatAll("今の目標を完了するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は 1 分です");
		}
		else
		{
	...
	else if(g_iTimeLeft != 0)
	{
		...
		PrintToChatAll("今の目標を完了するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 分と %d 秒です", g_iTimeLeft/60, g_iTimeLeft%60)
	}
}
