...
public OnPluginStart()
{
	...
	AutoExecConfig(true, "nmrihstatssrv10");
...
public ConnectDatabase()
{
	...
	SQL_TQuery(hDatabase, T_FastQuery, "CREATE TABLE IF NOT EXISTS Tnmrihstats10 (steam_id VARCHAR(64) PRIMARY KEY, name TEXT, points INTEGER, kills INTEGER, deaths INTEGER);");
}
...
public OnMapStart()
{
	for(new i = 1; i <= MaxClients; i++)
	{
		if(IsClientAuthorized(i) && !IsFakeClient(i))
		{
			...
			Format(query, sizeof(query), "SELECT name, points, kills, deaths FROM Tnmrihstats10 WHERE steam_id = '%s' LIMIT 1;", auth);
...
public T_LoadPlayer(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		else
		{
			...
			{
				SQL_TQuery(hDatabase, T_AddPlayer, "SELECT AVG(points) FROM Tnmrihstats10;", client);
			}
			else
			{
				AddPlayer(client, 0);
...
public AddPlayer(client, points)
{
	...
	Format(query, sizeof(query), "INSERT INTO Tnmrihstats10 VALUES ('%s', '%s', %d, 0, 0);", authid, escname, points);
...
public ShowRank(client)
{
	...
	SQL_TQuery(hDatabase, T_UpdateTotalQuery, "SELECT COUNT(*) FROM Tnmrihstats10;");
	
	decl String:query[1024];
	Format(query, sizeof(query), "SELECT points FROM Tnmrihstats10 WHERE points > %d ORDER BY points ASC;", clientPoints[client]);
...
public ShowTop10(client)
{
	...
	SQL_TQuery(hDatabase, T_Top10Query, "SELECT name, points FROM Tnmrihstats10 ORDER BY points DESC LIMIT 10;", client);
}
...
{
	decl String:query[1024];
	Format(query, sizeof(query), "UPDATE Tnmrihstats10 SET points = %d WHERE steam_id = '%s';", points, auth);
	SQL_TQuery(hDatabase, T_FastQuery, query);
}
...
public Action:Command_Reset(client, args)
{
	...
	resetCode = 0;
	SQL_TQuery(hDatabase, T_FastQuery,"DELETE FROM Tnmrihstats10", 0, DBPrio_High);
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(attacker != 0 && attacker != client && !IsFakeClient(attacker) && IsClientAuthorized(attacker))
	{
		...
		Format(query, sizeof(query), "UPDATE Tnmrihstats10 SET points = points + %d WHERE steam_id = '%s';", change, authid);
		SQL_TQuery(hDatabase, T_FastQuery, query);
	...
	decl String:query[1024], String:authid[64];
	GetClientAuthString(client, authid, sizeof(authid));
	Format(query, sizeof(query), "UPDATE Tnmrihstats10 SET points = points + %d, deaths = deaths + 1 WHERE steam_id = '%s';", change, authid);
	SQL_TQuery(hDatabase, T_FastQuery, query);
...
public ZombieKilled(client)
{
	...
	decl String:query[1024], String:authid[64];
	GetClientAuthString(client, authid, sizeof(authid));
	Format(query, sizeof(query), "UPDATE Tnmrihstats10 SET points = points + %d, kills = kills + 1 WHERE steam_id = '%s';", change, authid);
	SQL_TQuery(hDatabase, T_FastQuery, query);
}
...
public UpdatePlayerName(const String:authid[], const String:name[])
{
	...
	Format(query, sizeof(query), "UPDATE Tnmrihstats10 SET name = '%s' WHERE steam_id = '%s';", escname, authid);
	SQL_TQuery(hDatabase, T_FastQuery, query);
