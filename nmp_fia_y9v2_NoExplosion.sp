...
#define PLUGIN_VERSION "1.3.4-20140215"
...
public Plugin:myinfo = {
	name = "[NMRiH] Full Infinite Ammo. add JAE-700.fix mac10.explotion=GrenadeOnly",
	author = "Leonardo. Modifier yomox9",
...
public OnPluginStart()
{
	...
	nmp_fia_version = CreateConVar( "nmp_fia_version", PLUGIN_VERSION, "NoMorePlugins Full Infinite Ammo Custom_y9v1", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_SPONLY|FCVAR_DONTRECORD );
	...
	HookConVarChange( nmp_fia_for_all = CreateConVar( "nmp_fia_for_all", bFIAForAll ? "1" : "0", "Inifite ammo for everyone.", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0 ), OnConVarChanged );
	...
	BuildPath( Path_SM, szFile, sizeof( szFile ), "data/nmrih_items_data_NoExplosion.txt" );
...
public OnConfigsExecuted()
{
	...
	ServerTag( "infinite_ammo", bFIAForAll );
	if( bFIAForAll )
	{
		new iEntity;
	...
	if( 0 < iClient <= MaxClients && IsClientInGame( iClient ) && IsPlayerAlive( iClient ) )
	{
		...
		else if( !( iButtons & IN_RELOAD ) && flReloadTime[iClient] >= 0.0 )
		{
			if( ( flCurTime - flReloadTime[iClient] ) <= 0.5 )
			{
				...
				if( ( nFIAState[iClient] == 1 || bFIAForAll && !bNoReload ) && iAmmoOffset > 0 && WeaponHasClip( iEntity ) && iClip >= 1 )
				{
					if( iClip1Offset > 0 )
...
public OnGameFrame()
{
	new iWeapon, iClip, iAmmoType, String:szClassname[21], Float:flNextPrimaryAttack, Float:flNextSecondaryAttack, Float:flNextBashAttack, Float:flCurTime = GetGameTime();
	for( new i = 1; i <= MaxClients; i++ )
		if( IsClientInGame( i ) && IsPlayerAlive( i ) )
		{
			...
			if( IsValidEdict( iWeapon ) )
			{
				...
				if( iClip >= 1 )
				{
					if( ( nFIAState[i] || bFIAForAll ) && iAmmoOffset > 0 && !WeaponHasClip( iWeapon ) )
					{
						iAmmoType = GetEntProp( iWeapon, Prop_Send, "m_iPrimaryAmmoType" );
						if( 0 <= GetEntData( i, iAmmoOffset + iAmmoType * 4 ) < iClip )
							SetEntData( i, iAmmoOffset + iAmmoType * 4, iClip, _, true );
					}
					if( ( nFIAState[i] == 2 || bFIAForAll && bNoReload ) && iClip1Offset > 0 && 0 <= GetEntData( iWeapon, iClip1Offset ) < iClip )
...
stock bool:WeaponHasClip( iEntity )
{
	...
	if( StrEqual( szClassname, "me_chainsaw" ) || StrEqual( szClassname, "tool_flare_gun" ) || StrContains( szClassname, "fa_" ) == 0 )
		return true;
