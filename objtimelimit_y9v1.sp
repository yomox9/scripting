...
/*public Action:Event_ObjectiveBegin(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintToChatAll("今の目標を達成するための(時間切れまでの)残り時間は %d 分と %d 秒です", g_iTimeLeft/60, g_iTimeLeft%60)
}*/
...
public Action:Event_ObjectiveComplete(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintToChatAll("先ほどの目標の達成、おめでとうございます")
...
public Action:Command_ObjTimeChng(iClient, sArgs)
{
	...
	if(!g_bObjMap)
	{
		...
		ReplyToCommand(iClient, "[SM] これはオブジェクティブマップではありません");
		return Plugin_Handled;
	}
	...
	if(g_iTimeLeft != -1)
	{
		...
		ShowActivity2(iClient, "[SM] ", "目標の残り時間を %d 分 %d 秒に変更しました", g_iTimeLeft/60, g_iTimeLeft%60);
	...
	else
	{
		...
		ShowActivity2(iClient, "[SM] ", "目標の残り時間は無効になりました");
...
public Action:Command_ObjTimeLeft(iClient, sArgs)
{
	...
	if(g_iTimeLeft != -1)
	{
		if(g_iTimeLeft/60 == 0 && g_iTimeLeft != 0)
		{
			...
			PrintToChat(iClient, "今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 秒です", g_iTimeLeft%60);
		}
		else if(g_iTimeLeft%60 == 0 && g_iTimeLeft != 0)
		{
			if(g_iTimeLeft/60 == 1)
			{
				...
				PrintToChat(iClient, "今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は 1 分です");
			}
			else
			{
				...
				PrintToChat(iClient, "今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 分です", g_iTimeLeft/60);
			}
		}	
		else if(g_iTimeLeft != 0)
		{
			...
			PrintToChat(iClient, "今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 分と %d 秒です", g_iTimeLeft/60, g_iTimeLeft%60)
		}
	}
	else
	{
		PrintToChat(iClient, "[SM] The National Guard has removed the time limit to complete this objective!");
		PrintToChat(iClient, "州兵はこの目標達成のタイムリミットを無くしました");
	}
...
public Action:Timer_TimeLeft(Handle:timer)
{
	if(g_iTimeLeft == 0)
	{
		...
		PrintToChatAll("時間切れとなり、ゾンビが地域にはびこりました");
		KillAlivePlayers();
		return Plugin_Stop;
	}
	...
	else if(g_iTimeLeft == -1)
	{
		...
		PrintToChatAll("州兵はこの目標達成のタイムリミットを無くしました");
		return Plugin_Stop;
	}
...
public Action:PrintTimeLeft()
{
	if(g_iTimeLeft/60 == 0 && g_iTimeLeft != 0)
	{
		...
		PrintToChatAll("今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 分です", g_iTimeLeft/60);
	}
	else if(g_iTimeLeft%60 == 0 && g_iTimeLeft != 0)
	{
		if(g_iTimeLeft/60 == 1)
		{
			...
			PrintToChatAll("今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は 1 分です");
		}
		else
		{
	...
	else if(g_iTimeLeft != 0)
	{
		...
		PrintToChatAll("今の目標を達成するための(州兵があなた達のサポートを取り止めて撤退するまでの)残り時間は %d 分と %d 秒です", g_iTimeLeft/60, g_iTimeLeft%60)
	}
}
