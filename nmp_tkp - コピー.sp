...
#define PLUGIN_VERSION			"1.2.2-20140221"
...
public OnPluginStart()
{
	...
	HookConVarChange( nmp_tkp_silent = CreateConVar( "nmp_tkp_silent", bSilentAdmin ? "1" : "0", _, FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_DONTRECORD|FCVAR_CHEAT, true, 0.0, true, 1.0 ), OnConVarChanged );
	...
	HookConVarChange( nmp_tkp_allow_revenge = CreateConVar( "nmp_tkp_allow_revenge", bAllowRevenge ? "1" : "0", _, FCVAR_PLUGIN, true, 0.0, true, 1.0 ), OnConVarChanged );
	
	AutoExecConfig( true, "plugin.nmp_tkp" );
	
	RegAdminCmd( "sm_tkp_reset", Command_ResetData, ADMFLAG_UNBAN, "Usage: sm_tkp_reset" );
	RegAdminCmd( "sm_tkp_resetc", Command_ResetClientData, ADMFLAG_UNBAN, "Usage: sm_tkp_resetc <targets>" );
...
public OnConfigsExecuted()
{
	...
	bAllowRevenge = GetConVarBool( nmp_tkp_allow_revenge );
}
...
public Event_PlayerDeath( Handle:hEvent, const String:szEventName[], bool:bDontBroadcast )
{
	...
	if( 0 < iPunisher[iVictim] <= MaxClients && bAllowRevenge )
	{
	...
	new String:szDetails[21], String:szTitle[121];
	...
	if( bAllowSlay )
	{
		...
		Format( szTitle, sizeof( szTitle ), "%t", "NMP Menu Opt Slay" );
		AddMenuItem( hPunishMenu[iVictim], szDetails, szTitle );
	}
	...
	if( bAllowBury )
	{
		...
		Format( szTitle, sizeof( szTitle ), "%t", "NMP Menu Opt Bury" );
		AddMenuItem( hPunishMenu[iVictim], szDetails, szTitle );
	}
	...
	if( bAllowInfect )
	{
		...
		Format( szTitle, sizeof( szTitle ), "%t", "NMP Menu Opt Infect" );
		AddMenuItem( hPunishMenu[iVictim], szDetails, szTitle );
	}
	...
	if( bAllowDisarm )
	{
		...
		Format( szTitle, sizeof( szTitle ), "%t", "NMP Menu Opt Disarm" );
		AddMenuItem( hPunishMenu[iVictim], szDetails, szTitle );
	}
	...
	DisplayMenu( hPunishMenu[iVictim], iVictim, iMenuTime );
}


public Action:Command_ResetData( iClient, nArgs )
{
...
public Action:Command_ResetClientData( iClient, nArgs )
{
	if( nArgs < 1 )
	{
		ReplyToCommand( iClient, "[SM] Usage: sm_tkp_resetc <targets>" );
		return Plugin_Handled;
	}
...
public Menu_Punishments( Handle:hMenu, MenuAction:iAction, iParam1, iParam2 )
	...
	{
		new String:szSelection[21], String:szBuffer[2][11], iTarget = 0, nNewPunishment = Punishment_None;
...
stock bool:PunishClient( iTargetUID, iNewPunisherUID = -1, nNewPunishment = Punishment_None )
{
	...
	else if( nTeamKills[iTarget] >= iMinKills )
	{
		...
		if( iBanDuration >= 0 )
		{
			...
			Format( szReason, sizeof( szReason ), "%T", "NMP Ban Message", LANG_SERVER, iBanDuration );
	...
	else
	{
		if( nTeamKills[iTarget] == ( iMinKills - 1 ) )
			PrintToChat( iTarget, "%s%t", PLUGIN_CHAT_PREFIX, iBanDuration >= 0 ? "NMP Ban Warn Message" : "NMP Kick Warn Message" );
