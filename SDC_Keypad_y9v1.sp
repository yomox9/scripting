...
public Plugin:myinfo = {
	...
	author = "Tast - SDC. Modified by yomox9",
	description = "Show input Password keypad. yomox9 customized",
	version = "1.1",
...
stock EndCode(const KeyPadID , const String:Code[] = "", const Correct = -1,const client = -1){
	...
	if(Correct != -1){
		...
		if(sdn_allpass){
			...
			PrintToChatAll("\x04%N\x01 Enter the code(コードを入力してください) \x04%s",client2,Code2)
		}
		else if(Correct == 1 && client2 != -1){
			PrintToChatAll("\x04%N\x01 Enter the correct password(正しいパスワードの入力) \x04%s",client2,Code2)
		}
		else if(Correct == 0 && client2 != -1){
			PrintToChatAll("\x04%N\x01 Enter the wrong password(間違ったパスワードの入力) \x04%s",client2,Code2)
		}
...
public Event_Keycode_Enter(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if(strlen(KeyPadCode) && sdn_cospass){
		...
		if(StrEqual(Code,KeyPadCode) || (StrEqual(Code,"0000") && StrEqual(KeyPadCode,"0"))){
			SetEventString(event, "code", CodeString);
			if(sdn_showpass) PrintToChatAll("\x04%N\x01 Enter the correct password(正しいパスワードの入力)",client)
		}
		else {
			if(sdn_showpass) PrintToChatAll("\x04%N\x01 Enter the wrong password(間違ったパスワードの入力) \x04%s",client,Code)
			SetEventString(event, "code", "95271");
		}
		return;
	}
	...
	if(sdn_showpass){
		if(StrEqual(Code,CodeString))
				PrintToChatAll("\x04%N\x01 Enter the correct password(正しいパスワードの入力) \x04%s",client,CodeString)
		else 	PrintToChatAll("\x04%N\x01 Enter the wrong password(間違ったパスワードの入力) \x04%s",client,Code)
	}
		
	//PrintToChatAll("\x04%N\x01 Enter the password\x04%s \x01%s",client,Code,CodeString)
}
