 ...
 * Version: $Id$
 */
//classicmode vote is based on funvotes, but is not integrated, to make updating easier
...
{
	name = "Fun Votes Classicmodevote",
	author = "AlliedModders LLC, Timiditas",
	description = "change mode to a classic mode after vote succeed",
...
public OnPluginStart()
{
	...
	LoadTranslations("classicmodevote.phrases");

	CreateConVar("classicmodevote_version", PLUGIN_VERSION, "Classicmode vote version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);

	RegAdminCmd("sm_voteclassicmode", Command_VoteClassicmode, ADMFLAG_VOTE, "sm_voteclassicmode");
	g_Cvar_Limit = CreateConVar("sm_vote_classicmode", "0.60", "percent required for successful classicmode vote.", 0, true, 0.05, true, 1.0);
	HookEvent("round_start", EventRoundStart);
	RegConsoleCmd("classic", Chathandler);
}
...
public Handler_VoteCallback(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		else
		{
			...
			ServerCommand("sv_difficulty classic");
			PrintToChatAll("[SM] %t", "Change mode to Classic mode after map load.");
		}
	}
...
DisplayVoteClassicModeMenu(client)
{
	if (IsVoteInProgress())
	{
	...
	SetMenuTitle(g_hVoteMenu, "Change Classic mode?");
...
public Action:Command_VoteClassicmode(client, args)
{
	if (args > 0)
	{
		ReplyToCommand(client, "[SM] Usage: sm_classic");
