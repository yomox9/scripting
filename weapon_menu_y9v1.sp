...
new g_iSmgshotgunCount[MAXPLAYERS + 1];
new g_iGunCount[MAXPLAYERS + 1];
new g_iSetCount[MAXPLAYERS + 1];
...
new Handle:g_hSmgshotgunLimit = INVALID_HANDLE;
new Handle:g_hGunLimit = INVALID_HANDLE;
new Handle:g_hSetLimit = INVALID_HANDLE;
new Handle:g_hWeaponAdminOverride = INVALID_HANDLE;

public Plugin:myinfo = { name = "[NMRiH] Weapon Menu test", author = "Marcus Modified yomox9", description = "Allows players to select their own weapons to kill zombies.", version = Plugin_Version, url = "http://www.sourcemod.com"};
...
public OnPluginStart()
{
	...
	g_hSmgshotgunLimit = CreateConVar("sv_weaponmenu_smgshotgun_limit", "1", "Sets the maximum amount of times a smg/shotgun can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hSmgshotgunLimit, OnSettingsChange);
	g_hGunLimit = CreateConVar("sv_weaponmenu_gun_limit", "10", "Sets the maximum amount of times a Gun can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hGunLimit, OnSettingsChange);
	g_hSetLimit = CreateConVar("sv_weaponmenu_set_limit", "1", "Sets the maximum amount of times a Set can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hSetLimit, OnSettingsChange);
	...
	g_iSmgshotgunLimit = GetConVarInt(g_hSmgshotgunLimit);
	g_iGunLimit = GetConVarInt(g_hGunLimit);
	g_iSetLimit = GetConVarInt(g_hSetLimit);
...
public OnClientPostAdminCheck(iClient)
{
	if (1 <= iClient <= MaxClients)
	{
		...
		g_iSmgshotgunCount[iClient] = 0;
		g_iGunCount[iClient] = 0;
		g_iSetCount[iClient] = 0;
...
//		else
	if (hConvar == g_hSmgshotgunLimit) g_iSmgshotgunLimit = StringToInt(sNewValue);
//		else
	if (hConvar == g_hGunLimit) g_iGunLimit = StringToInt(sNewValue);
//		else
	if (hConvar == g_hSetLimit) g_iSetLimit = StringToInt(sNewValue);
}
...
public Action:Command_WeaponMenu(iClient, iArgs)
{
	if (IsPlayerAlive(iClient))
	{
		...
		AddMenuItem(hMenu, "0", "Gun/Grenade 銃   ",((g_iGunLimit > 0)		&& (g_iGunCount[iClient] < g_iGunLimit))			&& (ItemQuantity("gun") > 0) ?		ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "1", "Melee 近接武器   ",((g_iMeleeLimit > 0)	&& (g_iMeleeCount[iClient] < g_iMeleeLimit))		&& (ItemQuantity("melee") > 0) ?	ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "2", "Medical 医療品   ",((g_iMedicalLimit > 0)	&& (g_iMedicalCount[iClient] < g_iMedicalLimit))	&& (ItemQuantity("medic") > 0) ?	ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "3", "Tool 道具        ",((g_iToolLimit > 0)		&& (g_iToolCount[iClient] < g_iToolLimit))			&& (ItemQuantity("tool") > 0) ?		ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "4", "Set セット       ",((g_iSetLimit > 0)		&& (g_iSetCount[iClient] < g_iSetLimit))			&& (ItemQuantity("set") > 0) ?		ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
...
//		AddMenuItem(hMenu, "8", "SMG/Shotguns",		((g_iSmgshotgunLimit > 0)		&& (g_iSmgshotgunCount[iClient] < g_iSmgshotgunLimit))			&& (ItemQuantity("s_mgshotgun") > 0) ?	ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		...
		case MenuAction_Select:
		{
			...
			switch (StringToInt(sInfo))
			{
				case 0:
				{
					...
					SetMenuTitle(hMenu, "Weapon Menu: Gun/Grenade 銃");

					for (new i; i < g_iWeapons; i++)
					{
						if (StrContains(g_sWeaponEntityCategory[i], "gun", false) == -1) continue;
				...
				{
					hMenu = CreateMenu(Weapon_Menu);
					SetMenuTitle(hMenu, "Weapon Menu: Melee 近接武器");
				...
				{
					hMenu = CreateMenu(Weapon_Menu);
					SetMenuTitle(hMenu, "Weapon Menu: Medical 医療品");
				...
				{
					hMenu = CreateMenu(Weapon_Menu);
					SetMenuTitle(hMenu, "Weapon Menu: Tool 道具");
				...
				{
					hMenu = CreateMenu(Weapon_Menu);
					SetMenuTitle(hMenu, "Weapon Menu: Set セット");

					for (new i; i < g_iWeapons; i++)
					{
						if (StrContains(g_sWeaponEntityCategory[i], "set", false) == -1) continue;
				...
				{
					hMenu = CreateMenu(Weapon_Menu);
					SetMenuTitle(hMenu, "Weapon Menu: SMG/Shotguns");

					for (new i; i < g_iWeapons; i++)
					{
						if (StrContains(g_sWeaponEntityCategory[i], "s_mgshotgun", false) == -1) continue;
...
public Weapon_Menu(Handle:hHandle, MenuAction:action, param1, param2)
{
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			...
			if (g_bAdminOverride && !g_bIsAdmin[param1])
			{
//				switch (action)
//				{
//					case 0:
//					{
//				 		g_iGunCount[param1] += 1;
//				 	}
//					case 1:
//					{
//						g_iMeleeCount[param1] += 1;
//					}
//					case 2:
//					{
//						g_iMedicalCount[param1] += 1;
//					}
//					case 3:
//					{
//						g_iToolCount[param1] += 1;
//					}
//					case 4:
//					{
//						g_iSetCount[param1] += 1;
//					}
//					case 5:
//					{
//						g_iMeleeCount[param1] += 1;
...
//					case 7:
//					{
//						g_iExplosiveCount[param1] += 1;
//					}
//					case 8:
//					{
//						g_iSmgshotgunCount[param1] += 1;
//					}
//				}
			}
...
Config_LoadWeapons() // Originally, this was from TwistedPanda's BuildWars : It helped alot!!
{
	...
	BuildPath(Path_SM, sPath, PLATFORM_MAX_PATH, "configs/weapon_menu_y9v1.ini");
