...
#define PLUGIN_VERSION			"1.0.0.13"
...
static const String:g_szConfigFile[] = "sourcemod/plugin.radio.cfg";
...
GetRadioStationsFromFile ()
{
    ...
    if (GetConVarInt (g_cVarLogging))
        LogMessage ("Loading the radio stations from \"%s\"", STATIONSFILE);
    ...
    if ((hMapFile = OpenFile (STATIONSFILE, "r")) != INVALID_HANDLE)
    {
        ...
        if (GetConVarInt (g_cVarLogging))
            LogMessage ("Finishing parsing \"%s\" - Found %d radio stations", STATIONSFILE, iNumOfStations);
        return;
    }
    
    if (GetConVarInt (g_cVarLogging))
        LogMessage ("Unable to open \"%s\"", STATIONSFILE);
