...
{
	name = "Country Nick Plugin modified yomox9",
	author = "Antoine LIBERT aka AeN0 / Modified yomox9",
...
public OnClientPutInServer(client)
{
	...
	if(!IsFakeClient(client) && client != 0)
	{
		...
		{
			PrintToChatAll("\x04%T", "Announcer country found", LANG_SERVER, client, country);
		}
		else
		{
			GetClientAuthString(client, buffer, sizeof(buffer));
			if(strcmp(buffer, "STEAM_0:0:83378854", false)||strcmp(buffer, "STEAM_0:0:50657886", false))
			{
				PrintToChatAll("\x04%T", "Announcer country found", LANG_SERVER, client, "Japan");
			}
			else
			{
				PrintToChatAll("\x04%T", "Announcer country not found", LANG_SERVER, client);
